...
export class BalanceController {
  ...
  @ApiResponse({ status: 400, description: 'Validation failed - insufficient balance, missing verification, pending request exists, etc.' })
  ...
  @ApiResponse({ status: 400, description: 'Cannot cancel - request not found, already processed, or unauthorized' })
  ...
  @ApiResponse({ status: 200, description: 'Balance transaction created successfully' })
  @ApiResponse({ status: 400, description: 'Direct withdrawal not allowed for real account - use withdrawal request' })
  createBalanceEntry(
    @CurrentUser('sub') userId: string,
  ...
  async getRealBalance(@CurrentUser('sub') userId: string) {
    const balance = await this.balanceService.getCurrentBalance(userId, BALANCE_ACCOUNT_TYPE.REAL);
    return {
      accountType: 'real',
      balance,
    };
  ...
  async getDemoBalance(@CurrentUser('sub') userId: string) {
    const balance = await this.balanceService.getCurrentBalance(userId, BALANCE_ACCOUNT_TYPE.DEMO);
    return {
      accountType: 'demo',
      balance,
    };
  ...
  @Get('real/history')
  @ApiOperation({ summary: 'Get REAL account transaction history' })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  getRealBalanceHistory(
    ...
    return this.balanceService.getBalanceHistory(userId, queryDto, BALANCE_ACCOUNT_TYPE.REAL);
  }

  @Get('demo/history')
  @ApiOperation({ summary: 'Get DEMO account transaction history' })
  @ApiQuery({ name: 'page', required: false, type: Number })
  @ApiQuery({ name: 'limit', required: false, type: Number })
  getDemoBalanceHistory(
    ...
    return this.balanceService.getBalanceHistory(userId, queryDto, BALANCE_ACCOUNT_TYPE.DEMO);
  }
